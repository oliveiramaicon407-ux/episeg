          ...
          <div class="form-group">
            ...
            <select v-model="form.epi_id" class="custom-select">
              ...
              <option v-for="e in epis" :key="e.id" :value="e.id">
                {{ e.equipamentos }} (Saldo: {{ estoqueMap[e.id] ?? 0 }})
              </option>
            </select>
          </div>
        </div>
        ...
        <table class="styled-table">
          ...
          <tbody>
            <tr v-for="e in entregas" :key="e.id">
              ...
              <td>
                <div class="text-bold">{{ e.epis?.equipamentos }}</div>
...
async function carregar() {
  ...
  try {
    ...
    const { data: episData } = await supabase.from('epis').select('*').order('equipamentos')
    epis.value = episData || []
    ...
    const { data: entData } = await supabase
      .from('entregas')
      .select('*, funcionarios(nome, setor), epis(equipamentos, ca)')
...
.btn-primary:hover { background-color: #2c2c2c; transform: scale(1.05); }
.checkbox-group { display: flex; align-items: center; }
.checkbox-label { display: flex; align-items: center; gap: 8px; font-size: 0.85rem; color: #475569; }
